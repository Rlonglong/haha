...
| 角色 | 位置 | 負責什麼 | 碰得到資料嗎 |
|---|---|---|---|
| **VM4（Dagster）** | 跑 webserver / daemon / code server 的容器 | 排程、判斷相依性、發指令、收 log | ❌ 不碰檔案內容<br/>（唯一例外：對 SQL Server 下 index/partition 的 DDL） |
...
**目前規模**：15 張來源表、1 張 DB 同步表、37 支 dbt 模型 + 2 支中介表、2 支快照、7 個匯出。
...
放在 VM1 的 `/home/bcp_runner/scripts/`，**不在版控內**：
