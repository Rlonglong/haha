...
WITH DATE_VARS AS (
    SELECT 
        CAST('{{ target_date }}' AS DATE) AS TODAY_DT
...
, T_TXN_PS_BASE AS (
    -- [昨天] 的完整資料
    SELECT
        ACT_NO, CPU_DATE, TXN_TIME, FUNC_CODE, DR_FLG
        , CAST(TXN_AMT AS BIGINT) AS TXN_AMT
        , CAST(PS_BAL AS BIGINT)  AS PS_BAL
        , V.TODAY_DT, V.YESTERDAY_DT
    FROM {{ ref('txn_ps_net') }}
    CROSS JOIN DATE_VARS V
    WHERE (CPU_DATE = V.YESTERDAY_DT
            OR CPU_DATE = V.TODAY_DT)
      AND TABLE_DATE <= V.TODAY_DT
